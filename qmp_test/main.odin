...
run :: proc() -> int {
	...
	evs := client.events[:]
	check(len(evs) == 2, "events buffer after query-status holds 2 events")
	if len(evs) == 2 {
	...
	qmp.consume(&client, len(evs))

	// buffer cleared after consume
	check(len(client.events) == 0, "events buffer cleared after consume")
	...
	check(len(client.events) == 0, "no events after query-block")
	...
	check(len(client.events) == 1 && client.events[0].name == "RESET", "emit-event buffered RESET")
	qmp.consume(&client, 1)

	// poll with nothing pending: no error, reports nothing drained
	drained, perr := qmp.poll(&client, 100 * time.Millisecond)
	check(perr == .None, "poll with no data: no error (timeout is benign)")
	check(!drained, "poll with no data: reports nothing drained")
	check(len(client.events) == 0, "poll with no data: buffer stays empty")
	...
	qmp.consume(&client, 1)
	check(len(client.events) == 1 && client.events[0].name == "SPICE_INITIALIZED",
	      "consume(1) dropped only the oldest event")
	qmp.consume(&client, 9) // over-count clamps: drops the remaining event
	check(len(client.events) == 0, "consume(9) on 1 event: clamps, buffer empty")
